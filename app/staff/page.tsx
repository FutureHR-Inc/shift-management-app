...
function StaffPageContent() {
          ...
          try {
            ...
            if (response.ok) {
              ...
              if (latestUser && latestUser.company_id !== user.company_id) {
                ...
                console.log('🔄 [SYNC] LocalStorage updated with DB company_id:', latestUser.company_id);
              } else {
                setCurrentUser(user);
              }
            } else {
            ...
            console.error('🚨 [SYNC] Failed to sync user info:', syncError);
            setCurrentUser(user);
          }
        } catch (error) {
  ...
  // 🔧 ログイン用ID生成関数（プレビュー表示用）
  const generateLoginIdPreview = (name: string, stores: string[], role: 'manager' | 'staff') => {
    if (!name) return '';

    // 実際のIDはサーバー側でランダム生成されることを示す
    if (role === 'manager') {
      return 'MGR****'; // ランダム例: MGRX7Q2
    } else {
      return 'STF****'; // ランダム例: STFM3K9
    }
  };
          ...
          <div className="mt-6 border-b border-gray-200">
            <nav className="-mb-px flex space-x-8">
              <button
                ...
                className={`py-2 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'staff-list'
              ...
              {!currentUser?.company_id && (
                <button
                  ...
                  className={`py-2 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'company-registration'
                              ...
                              <div className="text-xs sm:text-sm text-gray-600 space-y-0.5">
                                ...
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className="font-mono text-blue-600 font-medium text-xs truncate">
                                    {user.loginId || generateLoginIdPreview(user.name, user.stores, user.role)}
                                  </span>
                                  <button
                                    onClick={() => {
                                      const loginId = user.loginId || generateLoginIdPreview(user.name, user.stores, user.role);
                ...
                >
                  <div className="p-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      {/* 🔑 ログインID表示（編集時のみ） */}
                      {editingUser && editingUser.loginId && (
                        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                          <div className="flex items-center space-x-2 mb-2">
                            <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m0 0a2 2 0 012 2m-2-2h-6m6 0v6a2 2 0 11-4 0V9a2 2 0 012-2z" />
                            </svg>
                            <h4 className="text-sm font-medium text-blue-800">ログイン情報</h4>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="bg-white border border-blue-200 rounded-lg px-3 py-2">
                                <code className="text-sm font-mono text-blue-900">{editingUser.loginId}</code>
                              </div>
                            </div>
                            <div>
                              <label className="block text-xs font-medium text-blue-700 mb-1">
                              ...
                              <div className="bg-white border border-blue-200 rounded-lg px-3 py-2">
                                <span className="text-xs text-blue-600">初回ログイン時にスタッフが設定</span>
                              </div>
                            </div>
                          </div>
                          <div className="mt-3 p-3 bg-blue-100 rounded-lg">
                            <p className="text-xs text-blue-700">
                              💡 <strong>スタッフへの案内:</strong> ログインID「<code className="font-mono bg-white px-1 rounded">{editingUser.loginId}</code>」を使用してログインしてください。初回ログイン時にパスワードの設定画面が表示されます。
                            </p>
                          </div>
                        </div>
                      )}
